...
# 1. Definimos a estrutura exata que queremos que a IA retorne usando Pydantic.
# Isso garante consistência nos dados e evita respostas aleatórias da IA.
...
def analisar_feedback_critico_com_ia(texto_original: str) -> dict:
    ...
    # Usamos temperatura 0.0 para que a IA seja o mais determinística/técnica possível
    llm = ChatGroq(
        temperature=0.0,
    ...
    # 4. Criamos o Prompt estruturado (System + Human)
    ...
    # 5. Montamos a Chain (Cadeia de execução) usando o LangChain Expression Language (LCEL)
    ...
    try:
        ...
        # Fallback de erro profissional caso a API falhe ou mude o formato
        return {
